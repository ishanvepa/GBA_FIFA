...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  ...
  for (int k = 0; k < height; k++) {
    DMA[3].src = &color;
    DMA[3].dst = videoBuffer + OFFSET(row + k, col, WIDTH);
...
//converts int into string then draws it
void drawInt(int row, int col, int value, u16 color) {
  // account for max nums 0->10
...
// eliminates background color for pngs to make transparent background
void drawImageWithTransparency(int row, int col, int width, int height, const u16 *image, u16 transparentColor) {
    for (int r = 0; r < height; r++) {
        for (int c = 0; c < width; c++) {
            u16 pixel = image[r * width + c];
